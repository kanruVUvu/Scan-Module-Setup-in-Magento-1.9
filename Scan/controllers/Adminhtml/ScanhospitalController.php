...
class Bms_Scan_Adminhtml_ScanhospitalController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		$model  = Mage::getModel('scan/scanhospital')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('scanhospital_data', $model);
			...
			$this->_addContent($this->getLayout()->createBlock('scan/adminhtml_scanhospital_edit'))
				->_addLeft($this->getLayout()->createBlock('scan/adminhtml_scanhospital_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('scan/scanhospital');		
			$model->setData($data)
        ...
        if(!is_array($scanIds)) {
            ...
            try {
                foreach ($scanIds as $scanId) {
                    $scan = Mage::getModel('scan/scanhospital')->load($scanId);
                    $scan->delete();
                }
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock('scan/adminhtml_scanhospital_grid')
            ->getCsv();
