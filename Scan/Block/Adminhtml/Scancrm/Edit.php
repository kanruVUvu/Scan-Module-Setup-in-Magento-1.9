...
class Bms_Scan_Block_Adminhtml_Scancrm_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        $this->_controller = 'adminhtml_scancrm';
        ...
        $this->_formScripts[] = "
            function toggleEditor() {
                if (tinyMCE.getInstanceById('scancrm_content') == null) {
                    tinyMCE.execCommand('mceAddControl', false, 'scancrm_content');
                } else {
                    tinyMCE.execCommand('mceRemoveControl', false, 'scancrm_content');
                }
            }
    ...
    {
        if( Mage::registry('scancrm_data') && Mage::registry('scancrm_data')->getId() ) {
            return Mage::helper('scan')->__("Edit Item '%s'", $this->htmlEscape(Mage::registry('scancrm_data')->getId()));
        } else {
